...
{
	//New code to scan outward
	int xs = (int)parts[ci].x, ys = (int)parts[ci].y, ds, is, y1, y2, x1, x2, ds2, cr;
	for(ds = 0; ds<sqrt(pow(XRES, 2)+pow(YRES, 2)); ds++){
		ds2 = ds%255;
		for(is = 0; is < ds+1; is++){
			x1 = xs - ds + is;
			y1 = ys - is;
			if(y1 < YRES && x1 < XRES && y1 > 0 && x1 > 0){
				cr = pmap[y1][x1]>>8;
				if(cr > 0 && cr < NPART){
					if(parts[cr].type == t && !parts[cr].life && ci != cr){
						return cr;
					}
				}
			}
