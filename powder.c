...
killed:
	    ...
	    if(t==PT_STKM)
	    {
		    ...
		    //Death
		    if(parts[i].life<1 || ((int)(player[0])&0x10) == 0x10 || (pv[y/CELL][x/CELL]>=4.5f && player[2] != SPC_AIR) )  //If his HP is less that 0 or there is very big wind...
		    {
			    player[0] = (int)(player[0])&15;
			    for(r=-2; r<=1; r++)
			    {
