...
wall_type * LoadWalls(int & wallCount)
{
	wall_type wtypes[] =
	{
		{PIXPACK(0x808080), PIXPACK(0x000000), 0, Renderer::WallIcon, "ERASE",			"DEFAULT_WL_ERASE",	L"Erases walls."},
		{PIXPACK(0xC0C0C0), PIXPACK(0x101010), 0, Renderer::WallIcon, "CONDUCTIVE WALL","DEFAULT_WL_CNDTW",	L"Blocks everything. Conductive."},
		{PIXPACK(0x808080), PIXPACK(0x808080), 0, Renderer::WallIcon, "EWALL",			"DEFAULT_WL_EWALL",	L"E-Wall. Becomes transparent when electricity is connected."},
		{PIXPACK(0xFF8080), PIXPACK(0xFF2008), 1, Renderer::WallIcon, "DETECTOR",		"DEFAULT_WL_DTECT",	L"Detector. Generates electricity when a particle is inside."},
		{PIXPACK(0x808080), PIXPACK(0x000000), 0, Renderer::WallIcon, "STREAMLINE",		"DEFAULT_WL_STRM",	L"Streamline. Set start point of a streamline."},
		{PIXPACK(0x8080FF), PIXPACK(0x000000), 1, Renderer::WallIcon, "FAN",			"DEFAULT_WL_FAN",	L"Fan. Accelerates air. Use the line tool to set direction and strength."},
		{PIXPACK(0xC0C0C0), PIXPACK(0x101010), 2, Renderer::WallIcon, "LIQUID WALL",	"DEFAULT_WL_LIQD",	L"Allows liquids, blocks all other particles. Conductive."},
		{PIXPACK(0x808080), PIXPACK(0x000000), 1, Renderer::WallIcon, "ABSORB WALL",	"DEFAULT_WL_ABSRB",	L"Absorbs particles but lets air currents through."},
		{PIXPACK(0x808080), PIXPACK(0x000000), 3, Renderer::WallIcon, "WALL",			"DEFAULT_WL_WALL",	L"Basic wall, blocks everything."},
		{PIXPACK(0x3C3C3C), PIXPACK(0x000000), 1, Renderer::WallIcon, "AIRONLY WALL",	"DEFAULT_WL_AIR",	L"Allows air, but blocks all particles."},
		{PIXPACK(0x575757), PIXPACK(0x000000), 1, Renderer::WallIcon, "POWDER WALL",	"DEFAULT_WL_POWDR",	L"Allows powders, blocks all other particles."},
		{PIXPACK(0xFFFF22), PIXPACK(0x101010), 2, Renderer::WallIcon, "CONDUCTOR",		"DEFAULT_WL_CNDTR",	L"Conductor. Allows all particles to pass through and conducts electricity."},
		{PIXPACK(0x242424), PIXPACK(0x101010), 0, Renderer::WallIcon, "EHOLE",			"DEFAULT_WL_EHOLE",	L"E-Hole. absorbs particles, releases them when powered."},
		{PIXPACK(0x579777), PIXPACK(0x000000), 1, Renderer::WallIcon, "GAS WALL",		"DEFAULT_WL_GAS",	L"Allows gases, blocks all other particles."},
		{PIXPACK(0xFFEE00), PIXPACK(0xAA9900), 4, Renderer::WallIcon, "GRAVITY WALL",	"DEFAULT_WL_GRVTY",	L"Gravity wall. Newtonian Gravity has no effect inside a box drawn with this."},
		{PIXPACK(0xFFAA00), PIXPACK(0xAA5500), 4, Renderer::WallIcon, "ENERGY WALL",	"DEFAULT_WL_ENRGY",	L"Allows energy particles, blocks all other particles."},
		{PIXPACK(0xDCDCDC), PIXPACK(0x000000), 1, Renderer::WallIcon, "AIRBLOCK WALL",	"DEFAULT_WL_NOAIR",	L"Allows all particles, but blocks air."},
		{PIXPACK(0x808080), PIXPACK(0x000000), 0, Renderer::WallIcon, "ERASEALL",		"DEFAULT_WL_ERASEA",L"Erases walls, particles, and signs."},
	};
...
menu_section * LoadMenus(int & menuCount)
{
	...
	{
		{"\xC1", L"Walls", 0, 1},
		{"\xC2", L"Electronics", 0, 1},
		{"\xD6", L"Powered Materials", 0, 1},
		{"\x99", L"Sensors", 0, 1},
		{"\xE2", L"Force", 0, 1},
		{"\xC3", L"Explosives", 0, 1},
		{"\xC5", L"Gasses", 0, 1},
		{"\xC4", L"Liquids", 0, 1},
		{"\xD0", L"Powders", 0, 1},
		{"\xD1", L"Solids", 0, 1},
		{"\xC6", L"Radioactive", 0, 1},
		{"\xCC", L"Special", 0, 1},
		{"\xD2", L"Game Of Life", 0, 1},
		{"\xD7", L"Tools", 0, 1},
		{"\xE4", L"Decoration tools", 0, 1},
		{"\xC8", L"Cracker", 0, 0},
		{"\xC8", L"Cracker!", 0, 0},
	};
	menuCount = SC_TOTAL;
