		...
		case PT_PAPR:
			{
				...
				auto weird = pt == PT_H2 || pt == PT_ANAR || pt == PT_BIZR || pt == PT_BIZRG;
				if (pt == PT_BCOL || bool(parts[ID(r)].life) == weird)
					result = 2;
				else
					result = 0;
			}
...
int Simulation::try_move(int i, int x, int y, int nx, int ny)
{
	...
	if (!e) //if no movement
	{
		...
		if (!legacy_enable && parts[i].type==PT_PHOT && r)//PHOT heat conduction
		{
			...
			if (rt < PT_NUM && !sd.IsHeatInsulator(parts[ID(r)]) && rt != PT_FILT && rt != PT_PAPR)
