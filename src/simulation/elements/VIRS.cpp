	...
	for (rx=-1; rx<2; rx++)
		for (ry=-1; ry<2; ry++)
		{
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				else if ((r&0xFF) != PT_VIRS && (r&0xFF) != PT_VRSS && (r&0xFF) != PT_VRSG && !(sim->elements[r&0xFF].Properties & PROP_NODESTRUCT) && ((r&0xFF) != PT_E189 || (rlife&~0x1) != 0x8 && (rlife != 16 || parts[r>>8].ctype != 4)))
				{
					if (!(rndstore & 0x7))
					{
