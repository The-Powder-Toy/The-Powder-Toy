        ...
        if(parts[i].type)
        {
            ...
            if(parts[i].life && t!=PT_ACID  && t!=PT_COAL && t!=PT_WOOD && t!=PT_NBLE && t!=PT_SWCH && t!=PT_STKM && t!=PT_FUSE && t!=PT_FSEP && t!=PT_BCOL && t!=PT_GOL && t!=PT_CRAC && t!=PT_DEUT)
            {
                if(!(parts[i].life==10&&(parts[i].type==PT_LCRY||parts[i].type==PT_PCLN||parts[i].type==PT_HSWC||parts[i].type==PT_PUMP)))
                    parts[i].life--;
                if(parts[i].life<=0 && t!=PT_METL && t!=PT_ARAY && t!=PT_IRON && t!=PT_FIRW && t!=PT_PCLN && t!=PT_HSWC && t!=PT_PUMP && t!=PT_WATR && t!=PT_RBDM && t!=PT_LRBD && t!=PT_SLTW && t!=PT_BRMT && t!=PT_PSCN && t!=PT_NSCN && t!=PT_NTCT && t!=PT_PTCT && t!=PT_BMTL && t!=PT_SPRK && t!=PT_LAVA && t!=PT_ETRD&&t!=PT_LCRY && t!=PT_INWR && t!=PT_GLOW && t!= PT_FOG && t!=PT_PIPE && t!=PT_FRZW &&(t!=PT_ICEI&&parts[i].ctype!=PT_FRZW)&&t!=PT_INST && t!=PT_SHLD1&& t!=PT_SHLD2&& t!=PT_SHLD3&& t!=PT_SHLD4&& t!=PT_DEUT)
                {
                    kill_part(i);
                    continue;
                }
                ...
                {
                    t = parts[i].ctype;
                    if(t!=PT_METL&&t!=PT_IRON&&t!=PT_BMTL&&t!=PT_BRMT&&t!=PT_LRBD&&t!=PT_RBDM&&t!=PT_BTRY&&t!=PT_NBLE&&t!=PT_DEUT)
										...
										if(t==PT_LASR)
{
	for(nx=-2;nx<3;nx++)
		...
		{		  
			...
			{
			r=pmap[y+ny][x+nx];
				if ((r&0xFF)==PT_SWCH) //Now we can melt this bugger...
				{
	...
	for(nx=-2;nx<3;nx++)
		...
		{		  
			...
			r=pmap[y+ny][x+nx];
				if ((r&0xFF)==PT_METL)
				{
					parts[i].type=PT_INWR;
					parts[r>>8].type=PT_INWR;
				}
			}
		}
}
	...
	for(nx=-2;nx<3;nx++)
		...
		{		  
			...
			r=pmap[y+ny][x+nx];
				if ((r&0xFF)==PT_SWCH)
				{
					parts[i].type=PT_PUMP;
					parts[r>>8].type=PT_PUMP;
				}
			}
		}
}
	...
	for(nx=-2;nx<3;nx++)
		...
		{		  
			...
			r=pmap[y+ny][x+nx];
				if ((r&0xFF)==PT_DSTW)
				{
					parts[i].type=PT_BIZR;
					parts[r>>8].type=PT_BIZR;
				}
			}
		}
}
										if(t==PT_VIRS)
{
	for(nx=-2;nx<3;nx++)
		...
		{		  
			...
			{
			r=pmap[y+ny][x+nx];
				if ((r&0xFF)==PT_FIRE||PT_PLSM||PT_SMKE)
				{
					parts[i].type=PT_VIRS;
	...
	for(nx=-2;nx<3;nx++)
		...
		{		  
			...
			r=pmap[y+ny][x+nx];
				if ((r&0xFF)==PT_ALPR)
				{
					parts[i].type=PT_ALBM;
					parts[r>>8].type=PT_ALBM;
				}
			}
		}
}
										...
										if(t==PT_CL)
{
	for(nx=-2;nx<3;nx++)
		...
		if(parts[i].temp>747.0f)
            ...
            if(t==PT_WATR || t==PT_ETRD || t==PT_SLTW || t==PT_METL || t==PT_IRON || t==PT_RBDM || t==PT_LRBD || t==PT_BRMT || t==PT_PSCN || t==PT_NSCN || t==PT_NTCT || t==PT_PTCT || t==PT_BMTL || t==PT_SPRK|| t == PT_NBLE || t==PT_INWR || t==PT_DEUT)
            {
                nx = x % CELL;
                if(nx == 0)
                    nx = x/CELL - 1;
                ...
                {
                    if(t==PT_WATR || t==PT_ETRD || t==PT_SLTW || t==PT_METL || t==PT_IRON || t==PT_RBDM || t==PT_LRBD || t==PT_NSCN || t==PT_NTCT || t==PT_PTCT || t==PT_PSCN || t==PT_BRMT || t==PT_BMTL||t==PT_NBLE || t==PT_INWR || t==PT_DEUT)
                    {
                ...
                for(nx=-2; nx<3; nx++)
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            if((r>>8)>=NPART || !r)
                                continue;
                            if(((r&0xFF)==PT_METL || (r&0xFF)==PT_IRON || (r&0xFF)==PT_ETRD || (r&0xFF)==PT_PSCN || (r&0xFF)==PT_NSCN || (r&0xFF)==PT_NTCT || (r&0xFF)==PT_PTCT || (r&0xFF)==PT_BMTL || (r&0xFF)==PT_RBDM || (r&0xFF)==PT_LRBD || (r&0xFF)==PT_BRMT || (r&0xFF)==PT_NBLE || (r&0xFF)==PT_INWR || (r&0xFF)==PT_DEUT) && parts[r>>8].ctype!=PT_SPRK)
                            {
                                t = parts[i].type = PT_NONE;
                ...
                for(nx=-2; nx<3; nx++)
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            {
                                if((rt==PT_METL||rt==PT_IRON||rt==PT_ETRD||rt==PT_BMTL||rt==PT_BRMT||rt==PT_LRBD||rt==PT_RBDM||rt==PT_PSCN||rt==PT_NSCN||rt==PT_NBLE||rt==PT_DEUT)&&parts[r>>8].life==0 && abs(nx)+abs(ny) < 4)
                                {
                                    parts[r>>8].life = 4;
                                    parts[r>>8].ctype = rt;
                                    parts[r>>8].type = PT_SPRK;
                    ...
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            if((a || ptypes[rt].explosive) && ((rt!=PT_RBDM && rt!=PT_LRBD && rt!=PT_INSL && rt!=PT_SWCH) || t!=PT_SPRK) &&
                                    ...
                                    (t!=PT_LAVA || parts[i].life>0 || (rt!=PT_STNE && rt!=PT_PSCN && rt!=PT_NSCN && rt!=PT_NTCT && rt!=PT_PTCT && rt!=PT_METL  && rt!=PT_IRON && rt!=PT_ETRD && rt!=PT_BMTL && rt!=PT_BRMT && rt!=PT_SWCH && rt!=PT_INWR && rt!=PT_DEUT)) && !(rt==PT_CRAC && parts[r>>8].life>0) &&
                            ...
                            if(legacy_enable)
                            {
                                if(t!=PT_SPRK && ptypes[rt].meltable  && ((rt!=PT_RBDM && rt!=PT_LRBD) || t!=PT_SPRK) && ((t!=PT_FIRE&&t!=PT_PLSM) || (rt!=PT_METL && rt!=PT_IRON && rt!=PT_ETRD && rt!=PT_PSCN && rt!=PT_NSCN && rt!=PT_NTCT && rt!=PT_PTCT && rt!=PT_BMTL && rt!=PT_BRMT && rt!=PT_SALT && rt!=PT_INWR && rt!=PT_DEUT)) &&
                            ...
                            if(pavg != PT_INSL)
                            {
                                if(t==PT_SPRK && (rt==PT_METL||rt==PT_IRON||rt==PT_ETRD||rt==PT_BMTL||rt==PT_BRMT||rt==PT_LRBD||rt==PT_RBDM||rt==PT_PSCN||rt==PT_NSCN||rt==PT_NBLE ||rt==PT_DEUT) && parts[r>>8].life==0 &&
                                ...
                                {
                                    if(rt==PT_METL||rt==PT_ETRD||rt==PT_BMTL||rt==PT_BRMT||rt==PT_LRBD||rt==PT_RBDM||rt==PT_PSCN||rt==PT_NSCN||rt==PT_DEUT)
                                    {
                                        t = parts[i].type = PT_ETRD;
                                        parts[i].ctype = PT_NONE;
                                        parts[i].life = 20;
