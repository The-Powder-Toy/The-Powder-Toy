...
int eval_move(int pt, int nx, int ny, unsigned *rr)
{
    ...
    if (rr)
        *rr = r;
    if (pt>=PT_NUM || parts[r>>PS].type>=PT_NUM)
        return 0;
    result = can_move[pt][r&TYPE];
    if (result==3)
    {
        if (pt==PT_PHOT && parts[r>>PS].type==PT_LCRY)
            result = (parts[r>>PS].life > 5)? 2 : 0;
        if (parts[r>>PS].type==PT_INVIS)
        {
...
int try_move(int i, int x, int y, int nx, int ny)
{
    ...
    if (parts[r>>PS].type==PT_BOMB && parts[i].type==PT_BOMB && parts[i].tmp == 1)
        e = 2;
    ...
    if (!e) //if no movement
    {
        ...
        if (parts[i].type==PT_NEUT && (parts[r>>PS].type==PT_CLNE || parts[r>>PS].type==PT_PCLN || parts[r>>PS].type==PT_BCLN || parts[r>>PS].type==PT_PBCN))
        {
            if (!parts[r>>PS].ctype)
                parts[r>>PS].ctype = PT_NEUT;
        }
        if (parts[r>>PS].type==PT_PRTI && (parts[i].type==PT_PHOT || parts[i].type==PT_NEUT))
        {
            int nnx, count;
            if (nx-x<0)
            {
    ...
    if ((parts[r>>PS].type==PT_WHOL||parts[r>>PS].type==PT_NWHL) && parts[i].type==PT_ANAR) //whitehole eats anar
    {
        parts[i].type=PT_NONE;
        if (!legacy_enable)
        {
    ...
    if (r && e<NPART)//the swap part, if we make it this far, swap
    {
        if (parts[i].type==PT_NEUT)
        {
            ...
            if ((s&TYPE) && (s&TYPE)<PT_NUM && !(parts[s>>PS].properties&PROP_NEUTPENETRATE))
...
#endif
{
    ...
    if (t==SPC_HEAT||t==SPC_COOL)
    {
        if ((pmap[y][x]&TYPE)!=PT_NONE&&(pmap[y][x]&TYPE)<PT_NUM)
        {
            ...
            {
                if ((pmap[y][x]&TYPE)==PT_PUMP || (pmap[y][x]&TYPE)==PT_GPMP)
                {
    ...
    if (t==PT_SPRK)
    {
        ...
        parts[pmap[y][x]>>PS].actas = PT_SPRK;
    ...
    else if (p==-2)//creating from brush
    {
        ...
        {
            if ((
                        (pmap[y][x]&TYPE)==PT_CLNE||
                        (pmap[y][x]&TYPE)==PT_BCLN||
                        ((pmap[y][x]&TYPE)==PT_PCLN&&t!=PT_PSCN&&t!=PT_NSCN)||
                        ((pmap[y][x]&TYPE)==PT_PBCN&&t!=PT_PSCN&&t!=PT_NSCN)
                    )&&(
                        t!=PT_CLNE&&t!=PT_PCLN&&
                        t!=PT_BCLN&&t!=PT_STKM&&
...
void update_particles_i(pixel *vid, int start, int inc)
{
                    ...
                    for ( nnx=0; nnx<36; nnx++)
                        for ( nny=0; nny<36; nny++)
                        {
                            if (ny+nny>0&&ny+nny<YRES&&nx+nnx>=0&&nx+nnx<XRES)
                            {
                                ...
                                if (!rt&&merule[nny][nnx]==1)
                                    create_part(-1,nx+nnx,ny+nny,PT_ME);
                                else if (!rt)
                                    continue;
                                else if (parts[rt>>PS].type==PT_ME&&merule[nny][nnx]==0)
                                    kill_part(rt>>PS);
    ...
    for (i=start; i<(NPART-starti); i+=inc)
        if (parts[i].type)
        {
            ...
            if (parts[i].life>0 && (parts[i].properties&PROP_LIFE_DEC))
            {
                // automatically decrease life
                parts[i].life--;
                if (parts[i].life<=0 && (parts[i].properties&(PROP_LIFE_KILL_DEC|PROP_LIFE_KILL)))
                {
            ...
            else if (parts[i].life<=0 && (parts[i].properties&PROP_LIFE_KILL))
            {
                // kill if no life
                kill_part(i);
                continue;
            ...
            else if(!(parts[i].properties & TYPE_SOLID))
            {
                pGravX += gravx[y/CELL][x/CELL];
                pGravY += gravy[y/CELL][x/CELL];
            }
            ...
            if (!legacy_enable)
            {
                if (y-2 >= 0 && y-2 < YRES && (parts[i].properties&TYPE_LIQUID))  //some heat convection for liquids
                {
                    r = pmap[y-2][x];
                ...
                if (!do_move(i, x, y, fin_xf, fin_yf))
                {
                    ...
                    else
                    {
                        ...
                        if (parts[i].falldown>1 && !ngrav_enable && gravityMode==0 && parts[i].vy>fabsf(parts[i].vx))
                        {
                            ...
                            {
                                if (((pmap[fin_y][j]&TYPE)!=t || bmap[fin_y/CELL][j/CELL])
                                ...
                                if (fin_y!=clear_y && ((pmap[clear_y][j]&TYPE)!=t || bmap[clear_y/CELL][j/CELL])
                                ...
                                if ((pmap[fin_y][j]&TYPE)!=t || (bmap[fin_y/CELL][j/CELL] && bmap[fin_y/CELL][j/CELL]!=WL_STREAM))
                                    break;
                            }
                            if (parts[i].vy>0)
                                r = 1;
                            ...
                            if (s==1)
                                ...
                                {
                                    if (((pmap[j][nx]&TYPE)!=t || bmap[j/CELL][nx/CELL]) && do_move(i, nx, ny, (float)nx, (float)j))
                                        break;
                        ...
                        else if (parts[i].falldown>1 && fabsf(pGravX*parts[i].vx+pGravY*parts[i].vy)>fabsf(pGravY*parts[i].vx-pGravX*parts[i].vy))
                        {
                            ...
                            for (j=0; j<rt; j++)
                            {
                                ...
                                if (nx<0 || ny<0 || nx>=XRES || ny >=YRES)
                                    break;
                                if ((pmap[ny][nx]&TYPE)!=t || bmap[ny/CELL][nx/CELL])
                                {
