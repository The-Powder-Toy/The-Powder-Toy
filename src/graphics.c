...
int draw_tool_xy(pixel *vid_buf, int x, int y, int b, unsigned pc)
{
    ...
    if ((b&TYPE) == PT_LIFE)
    {
        ...
        drawtext(vid_buf, x+14-textwidth((char *)gmenu[(b>>PS)&TYPE].name)/2, y+4, (char *)gmenu[(b>>PS)&TYPE].name, c, c, c, 255);
    }
    else if (b>=UI_WALLSTART && b<=UI_WALLSTART+UI_WALLCOUNT)
    {
        int ds = 0;
...
void draw_air(pixel *vid)
{
    ...
    for (y=0; y<YRES/CELL; y++)
        ...
        {
            if (cmode == CM_PRESS || cmode == CM_PREAWE)
            {
                if (pv[y][x] > 0.0f)
            ...
            else if (cmode == CM_VEL || cmode == CM_AWESOME)
            {
...
void draw_parts(pixel *vid)
{
    ...
    for (i = 0; i<=parts_lastActiveIndex; i++)
    {
        ...
        if (parts[i].type)
        {
            ...
            glBegin (GL_POINTS);
            glColor3ub (PIXR(parts[i].dcolour), PIXG(parts[i].dcolour), PIXB(parts[i].dcolour));
            glVertex2d (nx, ny);
            glEnd ();
        }
#else
        if (parts[i].type) //start of it
        {
            ...
            if (cmode!=CM_HEAT)
            {
                ...
                if (cmode==CM_NOTHING && t!=PT_PIPE && t!=PT_SWCH && t!=PT_LCRY && t!=PT_PIVS && t!=PT_HETR && t!=PT_PUMP && t!=PT_GPMP && t!=PT_PBCN && t!=PT_FILT && t!=PT_HSWC && t!=PT_PCLN && t!=PT_DEUT && t!=PT_WIFI && t!=PT_LIFE)//nothing display but show needed color changes
                {
                    if (t==PT_PHOT)
                    {
                        cg = 0;
                ...
                else if (cmode==CM_GRAD)
                {
                    ...
                    cr = sin(frequency*q) * 16 + PIXR(parts[i].dcolour);
                    cg = sin(frequency*q) * 16 + PIXG(parts[i].dcolour);
                    cb = sin(frequency*q) * 16 + PIXB(parts[i].dcolour);
                    if (cr>=255)
                        cr = 255;
                    if (cg>=255)
                        cg = 255;
                    ...
                    blendpixel(vid, nx, ny, cr, cg, cb, 255);
                         ...
                         t!=PT_PCLN && t!=PT_PUMP && t!=PT_PIVS && t!=PT_HSWC &&
                ...
                else if (t==PT_QRTZ || t==PT_PQRT)
                {
                    ...
                    if (parts[i].temp>(ptransitions[t].thv-800.0f))//hotglow for quartz
                    {
                        ...
                        cr = sin(frequency*q) * 226 + (z * 16 + PIXR(parts[i].dcolour));
                        cg = sin(frequency*q*4.55 +3.14) * 34 + (z * 16 + PIXR(parts[i].dcolour));
                        cb = sin(frequency*q*2.22 +3.14) * 64 + (z * 16 + PIXR(parts[i].dcolour));
                        if (cr>=255)
                            cr = 255;
                        if (cg>=255)
                            cg = 255;
                    ...
                    {
                        cr = z * 16 + PIXR(parts[i].dcolour);
                        cg = z * 16 + PIXG(parts[i].dcolour);
                        cb = z * 16 + PIXB(parts[i].dcolour);
                        cr = cr>255?255:cr;
                        cg = cg>255?255:cg;
                        cb = cb>255?255:cb;
                        blendpixel(vid, nx, ny, cr, cg, cb, 255);
                ...
                {
                    cr = PIXR(parts[i].dcolour) - parts[i].life*15;
                    cg = PIXG(parts[i].dcolour) - parts[i].life*15;
                    cb = PIXB(parts[i].dcolour) - parts[i].life*15;
                    if (cr<=50)
                        cr = 50;
                    if (cg<=50)
                        cg = 50;
                ...
                else if (t==PT_SUN)
                {
                    ...
                    for(r = 0; r < 4; r++)
                    {
                        ...
                        {
                            addpixel(vid, nx+nxo, ny+nyo, PIXR(parts[i].dcolour), PIXG(parts[i].dcolour), PIXB(parts[i].dcolour), 255-orbd[r]);
                            if(cmode == CM_FIRE && r == 1 || cmode == CM_PREAWE && r == 1 || cmode == CM_AWESOME && r == 1)
                            {
                        ...
                        addpixel(vid, nx, ny, PIXR(parts[i].dcolour), PIXG(parts[i].dcolour), PIXB(parts[i].dcolour), 200);
                    }
                }
                else if (t==PT_PRTI)
                {
                ...
                else if ((t==PT_BIZR||t==PT_BIZRG||t==PT_BIZRS)&&parts[i].ctype)
                {
                    ...
                    cg = cg>255?255:cg;
                    cb = cb>255?255:cb;
                    if (fabs(parts[i].vx)+fabs(parts[i].vy)>0 &&(cmode == CM_FIRE||cmode==CM_BLOB || cmode==CM_FANCY || cmode==CM_AWESOME || cmode==CM_PREAWE))
                    {
                        fg = 0;
                        fb = 0;
                        fr = 0;
                ...
                else if (t==PT_PIPE)
                {
                    ...
                    if ((parts[i].tmp&TYPE)>0 && (parts[i].tmp&TYPE)<PT_NUM)
                    {
                        cr = PIXR(ptypes[parts[i].tmp&TYPE].pcolors);
                        cg = PIXG(ptypes[parts[i].tmp&TYPE].pcolors);
                        cb = PIXB(ptypes[parts[i].tmp&TYPE].pcolors);
                    }
                    blendpixel(vid, nx, ny, cr, cg, cb, 255);
                }
                ...
                else if (t==PT_WTRV)
                {
                    if (cmode == CM_FIRE||cmode==CM_BLOB || cmode==CM_FANCY || cmode==CM_AWESOME || cmode==CM_PREAWE)
                    {
                        x = nx/CELL;
                        y = ny/CELL;
                        cg = PIXG(parts[i].dcolour)/3;
                        cb = PIXB(parts[i].dcolour)/3;
                        cr = PIXR(parts[i].dcolour)/3;
                        cg += fire_g[y][x];
                ...
                else if (parts[i].properties&PROP_HOT_GLOW && parts[i].temp>(ptransitions[t].thv-800.0f))
                {
                    float frequency = 3.1415/(2*ptransitions[t].thv-(ptransitions[t].thv-800.0f));
                    int q = (parts[i].temp>ptransitions[t].thv)?ptransitions[t].thv-(ptransitions[t].thv-800.0f):parts[i].temp-(ptransitions[t].thv-800.0f);
                    cr = sin(frequency*q) * 226 + PIXR(parts[i].dcolour);
                    cg = sin(frequency*q*4.55 +3.14) * 34 + PIXG(parts[i].dcolour);
                    cb = sin(frequency*q*2.22 +3.14) * 64 + PIXB(parts[i].dcolour);
                    if (cr>=255)
                        cr = 255;
                    if (cg>=255)
                        cg = 255;
                ...
                else if(t==PT_MGMA && parts[i].life)
                {
                    cr = parts[i].life * 2 + 0xE0;
                    cg = parts[i].life * 1 + 0x50;
                    cb = parts[i].life/2 + 0x10;
                ...
                else if (t==PT_LAVA || t==PT_SPRK || t==PT_MGMA)
                {
                    vid[ny*(XRES+BARSIZE)+nx] = parts[i].dcolour;
                    if (cmode == CM_FIRE||cmode==CM_BLOB || cmode==CM_FANCY || cmode==CM_AWESOME || cmode==CM_PREAWE)
                    {
                ...
                else if (t==PT_DLAY)
                {
                    ...
                    cr = PIXR(parts[i].dcolour)+stage;
                    cg = PIXG(parts[i].dcolour)+stage;
                    cb = PIXB(parts[i].dcolour)+stage;
                    if(cr>255)
                        cr = 255;
                    if(cg>255)
                        cg = 255;
            ...
            else //heat view
            {
                ...
                int caddress = restrict_flt((int)( restrict_flt(ttemp, 0.0f, /*MAX_TEMP*/9999+(-MIN_TEMP)) / ((/*MAX_TEMP*/9999+(-MIN_TEMP))/1024) ) *3, 0.0f, (1024.0f*3)-3);
            ...
            //blob view!
            if (cmode == CM_BLOB&&t!=PT_FIRE&&t!=PT_DWFM&&t!=PT_PLSM&&t!=PT_HFLM&&t!=PT_NONE&&t!=PT_ACID&&t!=PT_LCRY&&t!=PT_GLOW&&t!=PT_SWCH&&t!=PT_SMKE&&t!=PT_WTRV&&!(t==PT_FIRW&&parts[i].tmp==3)&&t!=PT_LIFE)
            {
                if (t==PT_PHOT)
                {
                    cg = 0;
            ...
            for (j=0; j<scl; j++)
                for (i=0; i<scl; i++)
                {
                    ...
                    else if (t)
                    {
                        if (t>=PT_NUM)
                            goto corrupt;
                        r += PIXR(parts[i].dcolour);
                        g += PIXG(parts[i].dcolour);
                        b += PIXB(parts[i].dcolour);
                        a ++;
                    }
                }
            if (a)
