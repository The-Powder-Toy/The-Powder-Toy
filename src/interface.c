...
{
    int index = 0, results = 0;
    modlist_e *new_modlist = NULL;
    modlist_e *current_item = NULL, *new_item = NULL;
    char *fname;
#if defined(WIN32) && !defined(__GNUC__)
    struct _finddata_t current_file;
    intptr_t findfile_handle;
    ...
    while(derp = readdir(directory))
    {
        ...
        {
            char *ext = fname+(strlen(fname)-4);
            if((!strncmp(ext, ".lua", 4)) && (search==NULL || strstr(fname, search)))
            {
                new_item = malloc(sizeof(modlist_e));
                new_item->filename = malloc(strlen(folder)+strlen(fname)+1);
                sprintf(new_item->filename, "%s%s", folder, fname);
                new_item->next = NULL;
                if(new_modlist==NULL)
                {
                    new_modlist = new_item;
                    new_item->prev = NULL;
                }
                else
                {
