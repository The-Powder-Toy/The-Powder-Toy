...
void Interface_DrawMenu()
{
    ...
    if(svf_open)
        Graphics_RenderText(58, YRES+(MENUSIZE-12), svf_name, c, c, c, 255);
    else
    ...
    if(svf_tags[0])
        Graphics_RenderMaxText(240, YRES+(MENUSIZE-12), 154, svf_tags, c, c, c, 255);
    else
    ...
    if(svf_login)
        Graphics_RenderText(XRES-104+BARSIZE/*406*/, YRES+(MENUSIZE-12), svf_user, 255, 255, 255, 255);
    else
...
void Interface_DrawToolbar(int i, int *sl, int *sr, int b, int bq, int mx, int my)
{
    ...
    {
        Graphics_RenderText(XRES-GetTextWidth((char *)mwalls[h-122].descs)-BARSIZE, sy-10, (char *)mwalls[h-122].descs, 255, 255, 255, 255);
    }
    else
    {
        ...
        for(gj=0; gj<GRID_Y; gj++)
            for(gi=0; gi<GRID_X; gi++)
            {
                ...
                if(search_scoreup[pos]>0||search_scoredown[pos]>0)
                {
                    ...
                    Renderer_FillRectangle(gx+(XRES/GRID_S)+4, gy-1+((YRES/GRID_S+3)/2)-nyu, 4, nyu, 57, 187, 57, 255);
                    Renderer_FillRectangle(gx+(XRES/GRID_S)+4, gy-2+((YRES/GRID_S+3)/2), 4, nyd, 187, 57, 57, 255);
...
int Interface_OpenUI(char *save_id, char *save_date)
{
    ...
    while(!sdl_poll())
    {
        ...
        if(active && http_async_req_status(http))
        {
            ...
            if(status == 200)
            {
                pixel *full_save = Graphics_PrerenderSave(data, data_size, &imgw, &imgh);
                save_pic = Graphics_RescaleImage(full_save, imgw, imgh, &thumb_w, &thumb_h, 2);
                data_ready = 1;
                free(full_save);
            }
            active = 0;
        ...
        if(info_ready && !hasdrawninfo) {
            ...
            for(cc=0; cc<info->comment_count; cc++) {
                ...
                if(ccy+52<YRES+MENUSIZE) //Try not to draw off the screen.
                    Renderer_DrawLine(50+(XRES/2)+2, ccy+52, XRES+BARSIZE-50, ccy+52, 100, 100, 100, 255);
            }
            hasdrawninfo = 1;
