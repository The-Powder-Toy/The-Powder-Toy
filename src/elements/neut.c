	...
	for (rx=-1; rx<2; rx++)
		for (ry=-1; ry<2; ry++)
			if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
			{
				...
				/*if(r&TYPE>1 && r&TYPE!=PT_NEUT && r&TYPE-1!=PT_NEUT && r&TYPE-1!=PT_STKM &&
				  (ptypes[r&TYPE-1].menusection==SC_LIQUID||
				  ptypes[r&TYPE-1].menusection==SC_EXPLOSIVE||
				  ptypes[r&TYPE-1].menusection==SC_GAS||
				  ptypes[r&TYPE-1].menusection==SC_POWDERS) && 15>(rand()%1000))
				  r&TYPE--;*/
			}
	return 0;
}
