				...
				if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
				{
					...
					if (parts[r>>PS].type==PT_SPRK) {
						...
						for (docontinue = 1, nxx = 0, nyy = 0, nxi = rx*-1, nyi = ry*-1; docontinue; nyy+=nyi, nxx+=nxi) {
							...
							if (!((r>>PS)>=NPART)) {
								if (!r) {
									int nr = create_part(-1, x+nxi+nxx, y+nyi+nyy, PT_LAZR);
									if (nr!=-1) {
									...
									if (parts[r>>PS].type==PT_LAZR&&parts[r>>PS].tmp==0) {//if it hits another LAZR that isn't red
										if (nyy!=0 || nxx!=0) {
									...
									} else if (parts[r>>PS].type==PT_LAZR&&parts[r>>PS].tmp==1) {//if it hits one that already was a long life, reset it
										parts[r>>PS].life = 1020;
										//docontinue = 1;
									}
									else if (parts[r>>PS].type!=PT_INWR && parts[r>>PS].type!=PT_LGUN && parts[r>>PS].type!=PT_WIFI && !(parts[r>>PS].type==PT_SWCH && parts[r>>PS].life>=10)) {
										if (nyy!=0 || nxx!=0) {
									...
									} else if (parts[r>>PS].type==PT_INWR || parts[r>>PS].type==PT_LGUN || parts[r>>PS].type==PT_WIFI || parts[r>>PS].type==PT_FILT || (parts[r>>PS].type==PT_SWCH && parts[r>>PS].life>=10)) {
										docontinue = 1;
									} else {
										docontinue = 0;
									}
