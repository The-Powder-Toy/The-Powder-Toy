						...
						for (docontinue = 1, nxx = 0, nyy = 0, nxi = rx*-1, nyi = ry*-1; docontinue; nyy+=nyi, nxx+=nxi) {
							...
							if (!((r>>8)>=NPART)) {
								if (!r) {
									...
									if ((r&0xFF)==PT_LAZR&&parts[r>>8].tmp==0) {//if it hits another LAZR that isn't red
										if (nyy!=0 || nxx!=0) {
									...
									} else if ((r&0xFF)==PT_LAZR&&parts[r>>8].tmp==1) {//if it hits one that already was a long life, reset it
										parts[r>>8].life = 1020;
										//docontinue = 1;
									}
									else if ((r&0xFF)!=PT_INWR && (r&0xFF)!=PT_LGUN && (r&0xFF)!=PT_WIFI && !((r&0xFF)==PT_SWCH && parts[r>>8].life>=10)) {
										if (nyy!=0 || nxx!=0) {
									...
									} else if ((r&0xFF)==PT_INWR || (r&0xFF)==PT_LGUN || (r&0xFF)==PT_WIFI || (r&0xFF)==PT_FILT || ((r&0xFF)==PT_SWCH && parts[r>>8].life>=10)) {
										docontinue = 1;
									} else {
										docontinue = 0;
									}
