...
int run_stickman(float* playerp, UPDATE_FUNC_ARGS)
{
  ...
  playerp[16] = pp;
    ...
    if (((int)(playerp[0])&0x01) == 0x01 && ((parts[r>>PS].type>=PT_NUM) || ptypes[r&TYPE].state != ST_GAS))
    {
    ...
    //Go right
    r = pmap[(int)(parts[i].y+10)][(int)(parts[i].x)];
    if (((int)(playerp[0])&0x02) == 0x02 && ((parts[r>>PS].type>=PT_NUM) || ptypes[r&TYPE].state != ST_GAS))
    {
        if (r>=PT_NUM || (ptypes[r&TYPE].state != ST_LIQUID
                          && parts[r>>PS].type != PT_LNTG))
        {
    ...
    if (((int)(playerp[0])&0x04) == 0x04 && (
                (pmap[(int)(playerp[8]-0.5)][(int)(playerp[7])]&TYPE)>=PT_NUM ||
                ptypes[pmap[(int)(playerp[8]-0.5)][(int)(playerp[7])]&TYPE].state != ST_GAS ||
                (pmap[(int)(playerp[16]-0.5)][(int)(playerp[15])]&TYPE)>=PT_NUM ||
                ptypes[pmap[(int)(playerp[16]-0.5)][(int)(playerp[15])]&TYPE].state != ST_GAS))
    {
    ...
    for (rx=-2; rx<3; rx++)
        for (ry=-2; ry<3; ry++)
            if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
            {
                ...
                {
                    playerp[2] = r&TYPE;  //Current element
                }
